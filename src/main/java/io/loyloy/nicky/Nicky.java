...
public class Nicky extends JavaPlugin
{
    ...
    private void setupConfig()
    {
        ...
        if(!config.isSet("nickcooldown")) {
            config.set("nickcooldown", 60);
        }

        if(!config.isSet("three_letters_not_met")) {
            config.set("three_letters_not_met", "&cYour nickname must start with the first 3 letters of your username!");
        }
        if(!config.isSet("cooldown")) {
            config.set("cooldown", "&cYou must wait {nickcooldown} seconds between use!");
        }
